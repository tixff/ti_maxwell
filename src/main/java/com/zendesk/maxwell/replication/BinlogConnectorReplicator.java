...
public class BinlogConnectorReplicator extends RunLoopProcess implements Replicator {
	 ...
	 * @param maxwellSchemaDatabaseName
	 * @param metrics
	 * @param start
	 * @param stopOnEOF
	 * @param clientID
	 * @param heartbeatNotifier
	 * @param scripting
	 * @param filter
	 * @param outputConfig
	 * @param bufferMemoryUsage
	 * @param config
	 * @author lixinlin
	 */
	public BinlogConnectorReplicator(
	...
	private boolean offsetFilter(RowMap row) {
		...
		if (binlog_files != null) {
			if (!binlog_files.contains(file)) {
				return true;
			} else if (config.getFirstBinlogFile().equals(file)) {
				if (!(offset >= Long.parseLong(this.config.offsetStart))) return true;
			} else if (config.getLastBinlogFile().equals(file)) {
				if (!(offset <= Long.parseLong(this.config.offsetEnd))) return true;
			}
		}
