...
public class MaxwellConfig extends AbstractConfig {
	...
	protected MaxwellOptionParser buildOptionParser() {
		...
		//creare by lixinlin
		parser.accepts("offset_start", "binlog start offset").withRequiredArg();
		parser.accepts("offset_end", "binlog end offset").withRequiredArg();
		parser.accepts("binlog_files", "binlog files split by comma").withRequiredArg();
	...
	public String getFirstBinlogFile() {
		return binlog_files.split(",")[0];
	}

	public String getLastBinlogFile() {
		String[] files = binlog_files.split(",");
